...
function defaultPractice() {
  return {
    ...
    rows: [
      // { id, voltage, run1, run2, run3, createdAt }
    ],
  };
}
...
let pTimer = {
  ...
  raf: null,
  activeRowId: null,
  activeRunIndex: 0, // 0..2 for run1..run3
};
...
function practiceStart() {
  ...
  // ensure active row exists (3 runs per row)
  if (!pTimer.activeRowId) {
    ...
    // ensure voltage stays as originally set for this row (but user can edit later in table)
  }
  ...
  $("#practice-voltage").disabled = true; // lock during a row capture
  practiceUpdateRunHint();
}
...
function practiceStop() {
  ...
  // reset stopwatch display for next run
  pTimer.elapsedMs = 0;
  $("#practice-time-display").textContent = fmtMs(0);

  // if finished 3 runs, release voltage lock and clear active row
  if (idx === 2) {
...
function practiceReset() {
  ...
  // Do not clear active row automatically; user might have started it
  practiceUpdateRunHint();
  practiceUpdateButtons();
}
...
function practiceRenderTable() {
  // sort by voltage descending, stable within equal voltage by createdAt
...
function editableCell(text, onCommit, isVoltage = false) {
  ...
  td.addEventListener("blur", () => {
    ...
    // allow blank to clear for runs; voltage cannot be blank
    if (!isVoltage && raw === "") {
      // clear run
      const ok = onCommit("0");
...
function practiceRenderChart() {
  ...
  if (!reg || points.length < 2) {
    ...
    const m = reg.m;
    const b = reg.b;
    $("#practice-eq").textContent = `Best fit: y = ${m.toFixed(4)}x + ${b.toFixed(4)}`;
...
function defaultMock() {
  return {
    ...
    period: {
      started: false,
      startAtISO: null,
      remainingMs: 8 * 60 * 1000, // 8 minutes
      paused: true,
      pauseAtPerf: null,
    },
    runState: {
      ...
      incompleteCount: 0,
      bonusEligible: false,
      finalRunType: "tt", // "tt" | "bonus"
      bonusUsed: false,
      ended: false,
    },
    runs: [
      // { idx, type, runtimeSec, status, distCm, ramp, touched, compViol, partFellOff, ds, ts, rs, bs }
    ],
  };
}
...
let mockRun = {
  ...
  elapsedMs: 0,
  raf: null,
  startedWhenPeriodRemainingMs: null,
  runStartValid: false, // started before period ended
};
...
function initMock() {
  ...
  $("#mock-did-not-cross-start").addEventListener("change", () => {
    ...
    // if this is checked, completion select disabled
  ...
  $("#mock-final-run-type").addEventListener("change", () => {
    mock.runState.finalRunType = $("#mock-final-run-type").value;
    saveMock(mock);
    mockRenderAll();
  });
...
function mockRenderPeriod() {
  // set display based on remainingMs
...
function mockStartPeriodRAF() {
  ...
  mockPeriodRAF = requestAnimationFrame(function tick() {
    ...
    // if time is 0, we don't auto-end; but runs can only start if started before 0
...
function mockUpdateRunButtons() {
  // can start run if period started and not paused and not ended by rule, and if still allowed attempts
  const canStart =
    mock.period.started &&
    !mock.period.paused &&
    !mock.runState.ended &&
  ...
  // if in record form, disable start
...
function mockStartRun() {
  ...
  // A run counts if it started before period ended
  const remaining = mock.period.remainingMs;
  mockRun.runStartValid = remaining > 0;
...
function mockStopRun() {
  ...
  // show run form to record outcome
  ...
  // final run type selection if bonus eligible
  $("#mock-final-run-type").disabled = !mock.runState.bonusEligible;
  $("#mock-final-run-type").value = mock.runState.finalRunType || "tt";

  // store temporary runtime for save
...
function mockRenderRunState() {
  ...
  // enforce ramp disabled when final run type is bonus
  const finalType = mock.runState.finalRunType || "tt";
  if (finalType === "bonus") {
    $("#mock-used-ramp").checked = false;
...
function mockSaveRun() {
  ...
  // if started after period ended, it should not count (per our “started before 0” rule)
  if (!startedValid) {
    $("#mock-save-hint").textContent = "This run started after the 8:00 timer hit 0, so it does not count.";
    $("#mock-save-hint").classList.remove("hidden");
    return;
  }

  // if did not cross start line within 3s -> does not count as a run
  ...
  if (partFellOff) compViol += 1; // treat as a competition violation too

  const completeMode = $("#mock-complete-select").value; // "complete" | "incomplete"
  let status = completeMode;

  // auto-incomplete checks (supervisor can still choose incomplete)
  // Incomplete if fails to move 3s before finish OR fails to cross finish within 2*TT.
  // We can't detect “fails to move”, so we enforce the 2*TT limit:
  const tt = mock.meta.tt;
  const btt = mock.meta.btt;
  const finalType = mock.runState.finalRunType || "tt";
  const target = (finalType === "bonus") ? btt : tt;

  if (target != null && runtimeSec > 2 * target) {
    status = "incomplete";
  }
  ...
  // Ramp usage not allowed for bonus
  let ramp = $("#mock-used-ramp").checked;
  if (finalType === "bonus") ramp = false;

  // Determine type label: TT or Bonus (only used for final run selection once eligible)
  const type = (mock.runState.bonusEligible && finalType === "bonus") ? "BONUS" : "TT";
  ...
  // update bonus eligibility after first complete TT run (per our implementation)
  if (type === "TT" && status === "complete" && mock.runState.completeCount === 1) {
    mock.runState.bonusEligible = isBonusEligible(runtimeSec, mock.meta.tt);
    if (mock.runState.bonusEligible) {
      // allow selection for final run
      $("#mock-final-run-type").disabled = false;
      mock.runState.finalRunType = $("#mock-final-run-type").value || "tt";
    }
  }

  // rules: after 1 complete run, only one more run allowed
  // and max 3 incomplete runs ends testing
  const reachedIncompleteLimit = mock.runState.incompleteCount >= 3;
  const reachedFinalAfterComplete = mock.runState.completeCount >= 1 && mock.runState.runCount >= 2; // exactly one more after first complete
  if (reachedIncompleteLimit || reachedFinalAfterComplete) {
    mock.runState.ended = true;
  }
...
function scoreRun({
  status, runtimeSec, tt, btt, finalRunType, distCm,
  ramp, touched, compViol, constructionViol, missedImpound
}) {
  // Multipliers: missed impound 0.7, construction 0.8^n, competition 0.9^n
  ...
  // Touch rule: complete but DS=0 and TS=0
  if (touched) {
    return { ds: 0, ts: 0, rs: 0, bs: 0, mult };
  }
  ...
  // Bonus scoring only for final run type BONUS (and must be complete to cross finish line)
  if (finalRunType === "BONUS") {
  ...
  // Apply multipliers to DS/TS/BS (as per our plan)
  ds *= mult;
  ts *= mult;
  rs = ds + ts;
  bs *= mult;
...
function mockRecalcAllScores() {
  // Re-score all stored runs if multipliers changed
  for (const r of mock.runs) {
    const scored = scoreRun({
      status: r.status,
...
function generateScoresheet() {
  mockRecalcAllScores(); // ensure up-to-date
  mockRenderScore();

  // Build HTML content for print-friendly sheet
